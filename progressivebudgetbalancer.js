...
function safeCalculateAdjustment(campaign, allCampaigns, factors) {
  try {
    if (!campaign) {
      return {
...
function main() {
  try {
    ...
    // Add this line to rank campaigns by performance
    rankCampaignsByPerformance(campaignData.campaigns);
...
/**
    ...
    // Apply trend factor and day-of-week adjustment first
  ...
  try {
    ...
    for (const campaign of campaignData.campaigns) {
      ...
      try {
        ...
        const adjustment = safeCalculateAdjustment(
          campaign, 
          campaignData.campaigns, // Pass all campaigns for relative comparison
          {
    ...
    // Only now apply impression share adjustment, based on campaign performance
    // This ensures we only consider impression share for campaigns already deemed worthy
    if (campaign.performanceScore >= 0.8) {
      const impressionShareFactor = calculateImpressionShareFactor(campaign, allCampaigns);
      adjustmentFactor *= impressionShareFactor;
      reasons.push(`Performance-weighted impression share factor: ${impressionShareFactor.toFixed(2)}`);
      
      // Add more confidence when we're using relative impression share adjustment
      confidence += 0.2;
    } else if (campaign.budgetImpressionShareLost > 70) {
        // Apply scaling if needed
        let finalBudgetAmount = budgetGroup.proposedAmount;
        if (scalingApplied) {
          // Old formula (only scales the increase):
...
function groupCampaignsByBudget(campaigns) {
  ...
  return { sharedBudgetGroups, individualCampaigns };
        ...
        // Validate adjustment values
        if (!adjustment || !adjustment.adjustmentFactor || isNaN(adjustment.adjustmentFactor)) {
      // Minimal flat adjustment for poor performers with severe budget limitations
      adjustmentFactor *= 1.05;
      reasons.push(`Minimal impression share adjustment (poor performer but severely limited): 1.05`);
    }
...
function calculateTrendFactor(campaign, allCampaigns) {
  try {
    // Check if this is a value-based strategy - set by calculateTrendFactorsForAll
    const isValueBased = campaign.isValueBasedStrategy || false;
    ...
    if (isValueBased) {
      ...
      // Replace the impression share factor calculation
      const impressionShareFactor = calculateImpressionShareFactor(campaign, allCampaigns);
 ...
 * Helper function to calculate impression share factor
 */
function calculateImpressionShareFactor(campaign, allCampaigns) {
  // Base factor - no adjustment
  let impressionShareFactor = 1.0;
  
  // Skip impression share consideration for low performers
  if (campaign.performanceScore < 0.8) {
    Logger.log(`Campaign ${campaign.name} has low performance score (${campaign.performanceScore.toFixed(2)}), using flat impression share factor`);
    
    // Minimal consideration for impression share lost
    if (campaign.budgetImpressionShareLost > 70) {
      impressionShareFactor = 1.05; // Tiny increase for severely limited campaigns
    } else {
      impressionShareFactor = 1.0;  // No increase for others
    }
    
    return impressionShareFactor;
  }
  
  // For good performers, calculate relative impression share factor
  
  // First get all campaigns with good performance
  const goodPerformers = allCampaigns.filter(c => c.performanceScore >= 0.8);
  
  // Find the maximum impression share lost among good performers
  const maxImpressionShareLost = Math.max(...goodPerformers.map(c => c.budgetImpressionShareLost || 0));
  
  // Calculate relative position within good performers
  const relativeImpressionShareLost = maxImpressionShareLost > 0 ? 
    campaign.budgetImpressionShareLost / maxImpressionShareLost : 0;
  
  // Scale the impression share factor based on relative position and performance rank
  // This gives highest weight to campaigns with both great performance and high impression share lost
  if (campaign.budgetImpressionShareLost > 10) {
    const performanceBoost = campaign.performanceRank * 0.5; // Up to 50% extra based on performance rank
    impressionShareFactor = 1.0 + (relativeImpressionShareLost * 0.2) + performanceBoost;
    
    Logger.log(`Campaign ${campaign.name}: High performer with ${campaign.budgetImpressionShareLost.toFixed(1)}% impression share lost`);
    Logger.log(`  - Performance rank: ${(campaign.performanceRank * 100).toFixed(1)}%`);
    Logger.log(`  - Relative impression share position: ${(relativeImpressionShareLost * 100).toFixed(1)}%`);
    Logger.log(`  - Impression share factor: ${impressionShareFactor.toFixed(2)}`);
  }
  
  // Cap the factor to avoid extreme adjustments
  return Math.min(1.5, impressionShareFactor);
}
...
function processCampaignBudgets(campaignData, dateRange, pacingInfo) {
  try {
    ...
    for (const campaign of campaignData.campaigns) {
      ...
      try {
        // Calculate adjustment with all available factors
        const adjustment = safeCalculateAdjustment(campaign, campaignData.campaigns, {
          trendFactor: campaign.trendFactor,
          dayOfWeekAdjustment: campaign.dayOfWeekAdjustment,
          performanceScore: campaign.performanceScore,
          efficiencyMetrics: campaign.efficiencyMetrics
...
function calculateTrendFactorsForAll(campaignData) {
  ...
  // Now calculate trend factors
  campaignData.campaigns.forEach(campaign => {
    campaign.trendFactor = calculateTrendFactor(campaign, campaignData.campaigns);
  });
}
...
// Add this function to rank campaigns by performance
function rankCampaignsByPerformance(campaigns) {
  // First, calculate performance scores for all campaigns
  campaigns.forEach(campaign => {
    // Calculate efficiency score (using either conversion value or volume)
    const isValueBased = campaign.isValueBasedStrategy || false;
    const efficiencyScore = isValueBased 
      ? (campaign.recentEfficiencyRatio || 0) / Math.max(0.1, campaign.longTermEfficiencyRatio || 0)
      : (campaign.recentEfficiencyRatio || 0) / Math.max(0.1, campaign.longTermEfficiencyRatio || 0);
    
    // Calculate recency score (using trend factor)
    const recencyScore = campaign.trendFactor || 1.0;
    
    // Combined performance score
    campaign.performanceScore = (efficiencyScore * 0.6) + (recencyScore * 0.4);
  });
  
  // Sort campaigns by performance score (descending)
  campaigns.sort((a, b) => b.performanceScore - a.performanceScore);
  
  // Assign relative rank (percentile) to each campaign
  const totalCampaigns = campaigns.length;
  campaigns.forEach((campaign, index) => {
    campaign.performanceRank = 1 - (index / Math.max(1, totalCampaigns - 1));
  });
